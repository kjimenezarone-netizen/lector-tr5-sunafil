...
const DRIVE_API_URL ="https://script.google.com/macros/s/AKfycbz8nX98_h3cad4oAujDLco2Y_U9MOfbNa4aCgrmVzEFA83AvbXb9xbx7WLG_4J326wk/exec";
...
// 7. DASHBOARD Y GOOGLE DRIVE
// ... (Funciones de navegación y proceso TR5 que ya tenemos) ...

async function renderDashboard() {
    const totalLabel = document.getElementById('stat-total');
    const topSedeLabel = document.getElementById('stat-top-sede');
    
    if (totalLabel) totalLabel.innerText = "Cargando...";

    try {
        // 1. Obtener datos desde Google Apps Script
        const response = await fetch(DRIVE_API_URL);
        const stats = await response.json(); // Formato esperado: {"LIMA": 10, "AREQUIPA": 5...}

        const labels = Object.keys(stats);
        const dataValues = Object.values(stats);
        
        // 2. Calcular Estadísticas Numéricas
        const total = dataValues.reduce((a, b) => a + b, 0);
        const topSede = labels.length > 0 ? labels[dataValues.indexOf(Math.max(...dataValues))] : "N/A";

        // Actualizar etiquetas en el HTML
        if (totalLabel) totalLabel.innerText = total;
        if (topSedeLabel) topSedeLabel.innerText = topSede;

        // 3. Crear o Actualizar Gráfico
        const ctx = document.getElementById('chart-intendencias').getContext('2d');
        if (window.myChart) window.myChart.destroy(); // Limpiar gráfico anterior
        ...
        window.myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: dataValues,
                    backgroundColor: 'rgba(0, 51, 102, 0.8)', // Azul Sunafil
                    borderColor: '#003366',
                    borderWidth: 1,
                    borderRadius: 10,
                    hoverBackgroundColor: '#d90000' // Rojo al pasar el mouse
                }]
            },
            options: {
                responsive: true,
        ...
        if (totalLabel) totalLabel.innerText = "Error";
    }
}
